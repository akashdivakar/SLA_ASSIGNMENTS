...
def create_clean_ppt_no_spellcheck():
    prs = Presentation()
    ...
    for slide_data in slides_content:
        ...
        # 6. Right Card (Side-by-Side Code with no proofing)
        ...
        # Split lines and disable spellchecking
        ...
        # Disable spellcheck on text frame XML so PowerPoint won't underline with red squiggles
        for p in tf_c.paragraphs:
            for r in p.runs:
                r._r.get_or_add_rPr().set('noProof', '1')

        # 7. Speaker Notes
        notes_slide = slide.notes_slide
        notes_slide.notes_text_frame.text = slide_data["notes"]

    output_path = r"c:\Users\akash\Desktop\SLA_Assignments\JS_Array_Methods_Clean_Seminar.pptx"
    prs.save(output_path)
    print(f"Clean presentation without spellcheck lines saved to: {output_path}")

if __name__ == "__main__":
    create_clean_ppt_no_spellcheck()
