...
def create_zero_red_lines_presentation():
    prs = Presentation()
    ...
    slides_content = [
        ...
        # Slide 10: Conclusion
        {
            ...
            "code": "// All Files in Folder:\n// 1. JS_Array_Methods_No_Red_Lines.pptx (Slide Deck)\n// 2. JS_Array_Methods_Seminar_Notes.md (Speaker Scripts)\n// 3. JS_Array_Methods_Seminar.html (Live Web Playground)",
    ...
    for slide_data in slides_content:
        ...
        # Disable proofing on all shapes on this slide so red lines NEVER appear
            ...
            if shape.has_text_frame:
                for paragraph in shape.text_frame.paragraphs:
                    for run in paragraph.runs:
                        ...
                        rPr.set('noProof', '1')
                        rPr.set('lang', 'zxx') # 'zxx' = No Linguistic Content (Completely disables spellcheck)

        # 7. Speaker Notes
        notes_slide = slide.notes_slide
        notes_slide.notes_text_frame.text = slide_data["notes"]

    temp_path = r"c:\Users\akash\Desktop\SLA_Assignments\temp_pres.pptx"
    prs.save(temp_path)

    # Post-process XML in pptx ZIP archive to add hideSpellingErrors and hideGrammarErrors
    final_output_path = r"c:\Users\akash\Desktop\SLA_Assignments\JS_Array_Methods_No_Red_Lines.pptx"
    
    with zipfile.ZipFile(temp_path, 'r') as zin:
        with zipfile.ZipFile(final_output_path, 'w', compression=zipfile.ZIP_DEFLATED) as zout:
            for item in zin.infolist():
                buffer = zin.read(item.filename)
                if item.filename.endswith(".xml") or item.filename.endswith(".rels"):
                    # Add noProof and lang="zxx" to any text run property in slides
                    xml_str = buffer.decode('utf-8')
                    # Replace run properties to ensure noProof='1'
                    xml_str = re.sub(r'<a:rPr([^>]*)>', r'<a:rPr\1 noProof="1" lang="zxx">', xml_str)
    ...
    print(f"100% Clean Presentation without any red lines saved to: {final_output_path}")

if __name__ == "__main__":
    create_zero_red_lines_presentation()
